...
{ config, wlib, lib, pkgs, ... }: {
  ...
  config.settings.colorscheme =
    "moonfly"; # <- just demonstrating that it is an option
  ...
  config.specs.general = {
    ...
    extraPackages = with pkgs; [
      ...
      # --- Fixes for snacks.explorer ---
      trash-cli # Provides 'trash' so deleting files goes to the recycle bin

      # --- Fixes for snacks.image ---
      ghostscript # Provides 'gs' for PDF rendering
      tectonic # Provides 'tectonic' for LaTeX rendering
      mermaid-cli # Provides 'mmdc' for Mermaid diagram rendering
    ];
  ...
  config.specMods = {
    ...
    parentOpts ? null, parentName ? null,
    ...
    ... }: {
      ...
      options.extraPackages = lib.mkOption {
        ...
        default = [ ];
        description =
          "a extraPackages spec field to put packages to suffix to the PATH";
      };
      # You could do this too
      # config.before = lib.mkDefault [ "INIT_MAIN" ];
    };
  config.extraPackages =
    config.specCollect (acc: v: acc ++ (v.extraPackages or [ ])) [ ];
